...
<form action="" method="post">
    <fieldset>
        ...
        <?php
        if (isset($_REQUEST["hi"]) && $_REQUEST["hi"] == 1) {
            echo "Приветствую, " . $_REQUEST["name"];
        }
        if (isset($_REQUEST["hi"]) && $_REQUEST["hi"] == 0) {
...
{
    $format = '<input type="hidden" name=%s value="0"><input type="checkbox" name=%s value="1">';
...
function check($name)
{
    $checked = !empty($_REQUEST[$name]) ? "checked" : "";
